...
router.post('/createnewuser', [
    ...
    try {
        ...
        if (user) {
            return res.status(400).json({success, errors: "Sorry email is already exists" });
        }
...
router.post('/login', [
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({success, errors: 'Please try to login with correct email id' });
        }
